...
export default function EditBusinessPage() {
  ...
  useEffect(() => {
    ...
    async function fetchBusiness() {
      try {
        ...
        setShareUrl(b.share_url || '');
        setShareMessage(b.share_message || '');
        setDiscountPercentage(b.discount_percentage.toString());
        setPhone(b.phone || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('An error occurred updating business.');
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-dvh bg-[#F8FAFC]">
      <header className="bg-white border-b border-[#E2E8F0]">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-[#0F172A]">Edit {name}</h1>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        <form onSubmit={handleSubmit} className="bg-white rounded-3xl p-6 sm:p-8 shadow-md border border-[#E2E8F0] space-y-7">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                />
                <p className="text-[11px] text-[#64748B] mt-1">URL: /s/{slug}</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-[#E2E8F0]">
              <div>
                ...
                <p className="text-xs text-[#64748B]">Toggle store active/disabled status.</p>
              </div>
              <button
                type="button"
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <span>Update Business</span>}
            </button>
          </div>
        </form>
      </main>
      ...
      {saved && (
        <div className="fixed inset-0 z-50 bg-[#0F172A]/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-3xl px-10 py-9 shadow-2xl text-center animate-scaleUp max-w-[320px] w-full">
            ...
            <p className="text-xs text-[#64748B] mt-1.5">{name.trim() || 'Business'}</p>
          </div>
        </div>
      )}
    </div>
